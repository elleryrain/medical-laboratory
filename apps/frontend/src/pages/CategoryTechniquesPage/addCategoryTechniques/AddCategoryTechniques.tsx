...
export function AddCategoryTechniques({
  toggleModal,
}: {
  toggleModal: () => void;
}) {
          ...
          <TechniquesInputContainer>
            {techniques.slice(0, count).map((technique) => (
              <TechniquesInputWrapper key={technique.id}>
                ...
                <TechniquesInputName>
                  {technique.lastName} {technique.firstName}{' '}
                  {technique.middleName}
                </TechniquesInputName>
              </TechniquesInputWrapper>
            ))}
            <TechniquesAddButton onClick={addCountHandler}>
              <Plus stroke="white" />
            </TechniquesAddButton>
          </TechniquesInputContainer>
        </TechniquesContainer>
        <PaymentTypeOfWorkContainer>
          ...
          {typesWork.map((typeWork) => (
            <TypeOfWorkContainer key={typeWork.id}>
              ...
              <InputContainer>
                {typesWorkStages
                  .filter((stage) => stage.typeWorkId === typeWork.id)
                  .map((stage, index) => (
                    <InputTypeOfWorkContainer key={stage.id}>
                      <InputTypeOfWorkStageContainer>
                        <InputTypeOfWorkStageCount>
                          {index + 1} этап
                        </InputTypeOfWorkStageCount>
                        <InputTypeOfWorkStageName>
                          {stage.nameTypeWorkStage}
                        </InputTypeOfWorkStageName>
                      </InputTypeOfWorkStageContainer>
