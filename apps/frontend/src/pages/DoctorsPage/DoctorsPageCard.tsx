...
const CardContainer = styled.div<{ $isOpen: boolean }>`
  ...
  background: #1c1c1c;
  padding: ${(props) =>
    props.$isOpen ? '30px 30px 30px 30px' : '12px 30px 13px 30px'};
  border-radius: 25px;
...
export function DoctorsPageCard() {
  ...
  const [editCardStates, setEditCardStates] = useState<{
    [key: number]: boolean;
  }>({});
        ...
        <CardContainer $isOpen={openCardStates[doctor.id]} key={doctor.id}>
          {!openCardStates[doctor.id] && (
            <CloseCardImgNameContainer>
              <CardImg
                $isOpen={openCardStates[doctor.id]}
                src={doctor.imgUrl}
                alt=""
              />
              <CardName
                $isOpen={openCardStates[doctor.id]}
              >{`${doctor.firstName} ${doctor.lastName} ${doctor.middleName}`}</CardName>
            </CloseCardImgNameContainer>
          )}

          <CardButtonContainer>
            <OpenSuccessCardButton
              onClick={() => buttonHandler('toggleCard', doctor.id)}
            >
            ...
            <OpenCardImgInfoContainer $isEdit={editCardStates[doctor.id]}>
              ...
              <OpenCardInfoContainer $isEdit={editCardStates[doctor.id]}>
                {editCardStates[doctor.id] ? (
                  <CardNameInput
                    onChange={(e) =>
                      updateDoctorNameHandler(doctor.id, e.target.value)
                    }
                    defaultValue={`${doctor.firstName} ${doctor.lastName} ${doctor.middleName}`}
                  />
                ) : (
                  <CardName
                    $isOpen={openCardStates[doctor.id]}
                ...
                <OpenCardClinicAdressContainer
                  $isEdit={editCardStates[doctor.id]}
                >
                  <OpenCardClinicAdressTitle
                    $isEdit={editCardStates[doctor.id]}
                  >
                    Клиника
                  </OpenCardClinicAdressTitle>
                  {editCardStates[doctor.id] ? (
                    <OpenCardClinicAdressNameInput
                      onChange={(e) =>
                        updateDoctorClinicHandler(doctor.id, e.target.value)
                      }
                      type="text"
                      defaultValue={doctor.nameClinic}
                    />
                  ) : (
                    <OpenCardClinicAdressName>
                      {doctor.nameClinic}
                    </OpenCardClinicAdressName>
                  )}
                </OpenCardClinicAdressContainer>
                ...
                >
                  ...
                  {editCardStates[doctor.id] ? (
                    <OpenCardClinicAdressNameInput
                      onChange={(e) =>
                        updateDoctorAdressHandler(doctor.id, e.target.value)
                      }
                      type="text"
