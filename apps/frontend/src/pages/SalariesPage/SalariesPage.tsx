        ...
        <div className="flex flex-wrap gap-7.5">
          {salariesStore.employees.map((employee) => {
            const technique = techniques.find(
              (t) => t.id === employee.techniqueId,
            );
            const category = technique
              ? categoryTechniques.find((c) => c.id === technique.categoryId)
                  ?.nameCategory
              : 'Не указана';
            const employeeSalaryStatus = salariesStore.salaryStatus.filter(
            ...
            return (
              <div key={employee.id}>
                <SalariesPageCard
                  ...
                  penalties={salariesStore.penalties.filter(
