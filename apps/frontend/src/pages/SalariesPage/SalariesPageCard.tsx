...
export const SalariesPageCard = ({
    ...
    const currentSalaryStatus = salaryStatus.filter((s) => s.employeeId === employeeId);
    const hasPaymentStatus = currentSalaryStatus.length > 0 ? currentSalaryStatus[0] : null;
    ...
    const getNewSalaryStatusId = () => {
        return salaryStatus.length > 0 ? Math.max(...salaryStatus.map((s) => s.id)) + 1 : 0;
    };
    ...
    const penaltiesSum = penalties.reduce((total, penalty) => total + penalty.amount, 0);
            ...
            <div className="bg-[#1C1C1C] w-[815px] px-[35px] pt-[35px] pb-[30px] rounded-[45px]">
                <div className="flex gap-5">
                    ...
                    <div className="flex flex-col justify-center gap-2">
                        <h1 className="text-2xl font-medium">{lastname} {firstname} {middlename}</h1>
                ...
                <div className="flex justify-between gap-[140px] items-center">
                    <p className="text-5xl font-medium text-nowrap">{formatNumber(fixedSalary + bonuses - penaltiesSum)} ₽</p>
                    <SalaryChart fixedSalary={fixedSalary} bonuses={bonuses} penalties={penaltiesSum} />
                </div>
                <p className="text-[#B5B5B5] mt-3 mb-5 text-2xl font-normal">Заработано</p>
                <div className="flex flex-wrap gap-2.5">
                    <div className="flex bg-[#2D2D2D] justify-between items-center pl-0.5 pr-3 py-0.5 rounded-full gap-2">
                        ...
                        <p className="font-medium text-[20px] text-nowrap">{formatNumber(fixedSalary)} ₽</p>
                    </div>
                    <div className="flex bg-[#2D2D2D] justify-between items-center pl-0.5 pr-3 py-0.5 rounded-full gap-2">
                        <div className="w-[40px] h-[40px] bg-[#65D691] rounded-full"></div>
                        <p className="font-normal text-[20px]">Наряды</p>
                        <p className="font-medium text-[20px] text-nowrap">{formatNumber(bonuses)} ₽</p>
                    </div>
                    <div className="flex bg-[#2D2D2D] justify-between items-center pl-0.5 pr-3 py-0.5 rounded-full gap-2 cursor-pointer" onClick={() => { openModal() }}>
                        <div className="w-[40px] h-[40px] bg-[#ED4C66] rounded-full"></div>
                        <p className="font-normal text-[20px]">Штрафы</p>
                        <p className="font-medium text-[20px] text-nowrap">{formatNumber(penaltiesSum)} ₽</p>
                    </div>
                </div>
                <p className="text-[#B5B5B5] mt-5 mb-3 text-2xl font-normal">Выполненные наряды</p>
                <WeeklyProgress techniqueId={techniqueId} />
                <Button title={isPaid ? `Выплачено ${buttonText}` : buttonText} theme={isPaid ? "salaryPaid" : "salaryNotPaid"} onClick={handlePayClick} className="mt-15 mx-auto" />
            </div>
