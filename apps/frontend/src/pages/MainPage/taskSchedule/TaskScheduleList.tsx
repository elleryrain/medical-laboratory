...
import TaskLine from '@svg/TaskLine.svg?react';
...
const TaskCompletedButton = styled.div<{
  ...
  border: ${(props) =>
    props.$isActive ? '2.5px solid #BDFF67' : '2.5px solid #5D5D5D'};
  cursor: pointer;
`;
...
export function TaskScheduleList() {
  ...
  const getCurrentTaskIndex = () => {
    ...
    for (let i = 0; i < tasks.length - 1; i++) {
      ...
      const [nextTaskHour, nextTaskMinute] = tasks[i + 1].time
        .split(':')
        .map(Number);
      ...
      if (
        currentTime >= taskTimeInMinutes &&
        currentTime < nextTaskTimeInMinutes
      ) {
            ...
            <TaskTimeLineContainer key={index}>
              <TaskTimeContainer>
                ...
                <TaskCompletedButton
                  ...
                  $isActive={taskTimeInMinutes <= currentTime}
                >
                  {task.isCompleted ? (
                    <CheckArrow stroke="black" height="17px" width="17px" />
                  ) : (
                    ''
                  )}
                </TaskCompletedButton>
              ...
              {index !== tasks.length - 1 && (
                <TaskLineStyled
                  stroke={taskTimeInMinutes < currentTime ? 'white' : '#919191'}
                />
              )}
        ...
        {tasks.map((task, index) => {
          ...
          return (
            <TaskNameLineContainer
              key={index}
              $isLineActive={
                index === currentTaskIndex && taskTimeInMinutes <= currentTime
              }
            >
              {index === currentTaskIndex &&
                taskTimeInMinutes <= currentTime && <TaskLineActive />}
